...
pub fn fetch_file(source_url: &str) -> Vec<u8> {
    let should_use_proxy = ["HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy"]
        .into_iter()
        .filter_map(|k| std::env::var(k).ok())
        .any(|v| !v.trim().is_empty());

    let resp = ureq::AgentBuilder::new()
        // Some environments export empty proxy vars (e.g. `HTTP_PROXY=`), which breaks ureq's
        // proxy parsing and results in DNS errors like resolving `:80`.
        .try_proxy_from_env(should_use_proxy)
        .build()
        .get(source_url)
